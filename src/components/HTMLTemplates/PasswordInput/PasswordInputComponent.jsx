...
<div class="tds-password-input${errorClass}" data-module="tds-password-input">
  ...
  <div class="tds-input__wrapper tds-password-input__wrapper${error ? ` tds-input--error` : ''}">
    <input class="tds-input-password tds-password-input__input${error ? ' tds-input-password--error' : ''}" type="${isOpened ? 'password' : 'text'}" id="${forAttribute}" name="${forAttribute}" spellcheck="false" autocomplete="current-password" autocapitalize="none"${valueAttribute} />
    ...
    <div class="tds-password-toggle-icon-wrapper">
      <button class="tds-button tds-button--icon-neutral tds-password-toggle-icon" type="button" data-module="tds-button" aria-controls="${forAttribute}" aria-label="${isOpened ? 'Show password' : 'Hide password'}">
        <span class="tds-icon">
          ${svgEyeOpened}
          <svg style="display: none;" xmlns="http://www.w3.org/2000/svg" class="eye-crossed" role="img" aria-label="eye" aria-hidden="true" width="22" height="20" viewBox="0 0 22 20" fill="none">
