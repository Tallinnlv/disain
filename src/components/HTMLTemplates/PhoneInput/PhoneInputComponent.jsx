...
const SelectComponent = ({
  ...
  const SvgEstoniaButton = html`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18" fill="none">
  ...
  const SvgEstonia = html`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18" fill="none">
              <path d="M24 0H0V17.9212H24V0Z" fill="black" />
              <path d="M24 12.0264H0V18.0001H24V12.0264Z" fill="white" />
              <path fill-rule="evenodd" clip-rule="evenodd" d="M0 0H24V5.97375H0V0Z" fill="#1291FF" />
            </svg>
  ...
  const SvgFinland = html`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18" fill="none">
  ...
  const SvgLatvia = html`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18" fill="none">
  ...
  const SvgLithuania = html`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18" fill="none">
  ...
  return html`
<div class="tds-form-group${modifiers.includes('is-error') ? ` tds-form-group--error` : ''}">
  ${title ? `<label class="tds-label${compact ? ' tds-label--compact' : ''}" for="phone-input">${title}${titleRequired ? required : ''}</label>` : ''}
  ${hint ? `<div id="phone-number-hint" class="tds-label__hint${compact ? ' tds-label__hint--compact' : ''}">${hint}</div>` : ''}
  ${modifiers.includes('is-error') ? `<div class="tds-error-message${compact ? ' tds-error-message--compact' : ''}">${errorMessage}</div>` : ''}
  <div class="tds-phone-form${modifiers.includes('is-error') ? ' tds-input--error' : ''}${isDisabled ? ' is-disabled' : ''}" id="${forAttribute}">
    ${showCountryCodeOnly ? '' : `
    <div class="tds-select-phone">
      <button class="tds-phone-control${compact ? ' tds-phone-control--compact' : ''}" aria-label="phone selection" aria-expanded="false" aria-controls="phone-dropdown"${isDisabled ? ' disabled' : ''}>
        <span class="tds-phone-control__placeholder">
          ${SvgEstoniaButton}
        </span>
        <span class="tds-select__icon${compact ? ' tds-select__icon--compact' : ''}">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path fillRule="evenodd" clipRule="evenodd" d="M3.25269 8.16436L4.7475 6.83563L12.0001 14.9948L19.2527 6.83563L20.7475 8.16436L12.0001 18.0052L3.25269 8.16436Z" fill="#1C1F22" />
          </svg>
        </span>
      </button>
      <ul
    ...
    <div class="tds-phone-field${modifiers.includes('is-error') ? ' tds-phone-field--error' : ''}${compact ? ' tds-phone-field--compact' : ''}">
      ...
      <input
        ...
        autocomplete="tel"${isDisabled ? ' disabled' : ''}
      />
    </div>
  </div>
</div>
