...
/**
 * @param {object} props
 * @param {string} [props.id] - Id of the tooltip element (generated when omitted)
 ...
 * @param {string} [props.buttonLabel='Muuda'] - Accessible name of the icon button trigger
 * @param {string} [props.triggerText='GDPR'] - Visible text of the text trigger
 */
...
const TooltipComponent = (props) => {
  ...
  const tooltipId = props.id ?? uid('tooltip', props);

  return html`
<div class="container">
  ${isButton
  ? `<button class="tooltip-target tds-button tds-button--icon-neutral" aria-label="${buttonLabel}" aria-describedby="${tooltipId}" data-position="${position}" type="button">
    <span class="tds-icon">
  ...
  ${isText ? `<span class="tooltip-target tds-body-md tds-color-content-default" tabindex="0" aria-describedby="${tooltipId}" data-position="${position}">${triggerText}</span>` : ''}
