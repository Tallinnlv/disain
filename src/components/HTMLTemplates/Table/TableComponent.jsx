...
const TableComponent = ({
  ...
  columns.forEach((column, index) => {
    tableHeadersHtml += `
        <th class="tds-table__header${alignRight && index % 2 !== 0 ? ' tds-table__header--right' : ''}${column.headerAlignRight ? ' tds-table__header--right' : ''}${column.sortable ? ' tds-table__header--sortable' : ''}" data-key="${column.key}" scope="col"${column.sortable ? ' data-sortable="true"' : ''}>
          <div>
            <span>${column.label}</span>
  
            ${column.sortable ? `<span class="tds-table__icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M8.47178 5.52837L4.66704 1.72363L0.862305 5.52837L1.80511 6.47118L4.00038 4.27592V13.3331H5.33371V4.27592L7.52897 6.47118L8.47178 5.52837ZM15.1384 10.4712L11.3337 14.2759L7.52897 10.4712L8.47178 9.52837L10.667 11.7236V2.66645H12.0004V11.7236L14.1956 9.52837L15.1384 10.4712Z" fill="#777A7E" />
              </svg>
            </span>` : ''}
          </div>
        </th>
  ...
  return html`
<div class="tds-table-container${compact ? ' tds-table-container--compact' : ''}">
  ${multiSelect ? `<div class="tds-table-container__multiselect-container">` : ''}
    ${caption ? `<div class="tds-table-container__caption">${caption}</div>` : ''}
    ${multiSelect ? `<div class="tds-table-container__selection">
      <span class="tds-table-container__selection-counter">2 rows selected</span>
      <button class="tds-button tds-button--compact-m tds-button--tertiary-neutral tds-button--icon-left" type="button">
  ...
  <table class="tds-table${compact ? ' tds-table--compact' : ''}${collapsed ? ' tds-table--collapsed' : ''}" id="${id}">
    ${hasHeader ? `<thead class="tds-table__head">
