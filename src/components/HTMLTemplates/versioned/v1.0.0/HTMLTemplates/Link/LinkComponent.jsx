...
function LinkComponent({
  ...
  const shouldRenderDiv = (bodySize && bodyClass) || content;

  return html` ${shouldRenderDiv
      ? `<div class="${bodyClass}">${content ? `<span class="tds-body-content">${content}</span>` : ''}`
      : ''}
    <a
      ...
      class="${!isIcon
        ? asButton
          ? 'tds-button'
          : 'tds-link'
        : ''} ${linkModifiers} ${!isTertiary && iconLeft
        ? asButton
          ? 'tds-button--icon-left'
          : 'tds-link--icon-left'
        : ''} ${!isTertiary && iconRight
        ? asButton
          ? 'tds-button--icon-right'
          : 'tds-link--icon-right'
        : ''} ${compact
        ? asButton
          ? 'tds-button--compact'
          ...
          ? html` <svg
            ...
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M16.5858 11L11.2929 5.70706L12.7071 4.29285L20.4142 12L12.7071 19.7071L11.2929 18.2928L16.5858 13H3V11H16.5858Z"
                fill="#0060AD"
              />
            </svg>`
          : ''}
