...
export function SelectComponent({
  ...
  return html`
    <div
      class="tds-form-group ${isError ? `tds-form-group--error` : ''} ${compact
        ? 'tds-form-group--compact'
        : ''}"
    >
      ${title
        ? `<label class="tds-label ${compact ? 'tds-label--compact' : ''}" for="location">${title}${required}</label>`
        : ''}
      ${hintTitle
        ? `<div id="location-hint" class="tds-label__hint ${compact ? 'tds-label__hint--compact' : ''}">${hintTitle}</div>`
        : ''}
      ${isError
        ? `<div class="tds-error-message ${compact ? 'tds-error-message--compact' : ''}">Error message</div>`
        : ''}
      <div class="tds-select ${isDisabled ? 'is-disabled' : ''}">
        <button
          class="tds-form-control ${isError
            ? 'tds-form-control--error'
            : ''}${compact ? 'tds-form-control--compact' : ''}"
          aria-label="Valikukast"
          aria-expanded="false"
          ${isDisabled ? 'disabled' : ''}
        >
          <span class="tds-form-control__placeholder"
            >${selectCustomText ? selectCustomText : 'Default option'}</span
          >
          <span
            class="tds-select__icon ${compact
              ? 'tds-select__icon--compact'
              : ''}"
            ><svg
        ...
        <ul
          class="tds-dropdown ${compact ? 'tds-dropdown--compact' : ''}"
          id="location"
