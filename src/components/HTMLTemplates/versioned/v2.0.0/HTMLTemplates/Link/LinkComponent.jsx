 ...
 * @param {boolean} [props.isDisabled = false] - Link disabled (rendered without href, with aria-disabled)
...
const LinkComponent = ({
  ...
  // A disabled link is not a link (APG): no href, but aria-disabled so the
  // state is announced. The classes stay so the disabled styling applies.
  const linkAttributes = isDisabled
    ? ' aria-disabled="true"'
    : ` href="${href}" target="_self"`;

  const linkHtml = html`
<a class="${!isIcon ? asButton ? 'tds-button' : 'tds-link' : ''} ${linkModifiers}${!isTertiary && iconLeft ? asButton ? ' tds-button--icon-left' : ' tds-link--icon-left' : ''}${!isTertiary && iconRight ? asButton ? ' tds-button--icon-right' : ' tds-link--icon-right' : ''}${compact ? asButton ? ' tds-button--compact' : ' tds-link--compact' : ''}${isDisabled ? ' is-disabled' : ''}"${linkAttributes}>
  ${customIconLeft
  ? customIconLeft
  : iconLeft
    ? html`<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false">
