 ...
 * @param {string} [props.label = 'Password'] - Visible label text
 * @param {boolean|string} [props.hint] - The hint text (`true` renders placeholder text)
 * @param {boolean|string} [props.error] - The error message (`true` renders placeholder text)
 * @param {string} [props.passwordFilled] - Prefilled value
 * @param {boolean} [props.isOpened = false] - Whether the password is visible (type="text") initially
 * @param {string} [props.autocomplete = 'current-password'] - Autocomplete token ('new-password' when creating one)
 * @param {string} [props.toggleLabel = 'Show password'] - Accessible name of the toggle button;
 *   its pressed state (`aria-pressed`) tells whether the password is currently shown
 * @param {string} [props.statusShown = 'Password is shown'] - Live-region text after showing the password
 * @param {string} [props.statusHidden = 'Password is hidden'] - Live-region text after hiding the password
 */

const eyeIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="eye" aria-hidden="true" focusable="false" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
const eyeCrossedIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="eye-crossed" aria-hidden="true" focusable="false" width="22" height="20" viewBox="0 0 22 20" fill="none">
...
const PasswordInputComponent = (props = {}) => {
  const {
    ...
    statusShown = 'Password is shown',
    statusHidden = 'Password is hidden',
  } = props;

  const id = props.id ?? uid('password-input', props);
  ...
  const inputAttributes = [
    `class="tds-input-password tds-password-input__input${error ? ' tds-input-password--error' : ''}"`,
    ...
    passwordFilled ? `value="${passwordFilled}"` : '',
  ]
    .filter(Boolean)
    .join(' ');

  // Both icons ship in the markup; the script toggles the `hidden` attribute on
  // their wrappers. The open eye is shown while the password is hidden.
  return html`
<div class="tds-password-input${error ? ' tds-form-group--error' : ''}" data-module="tds-password-input" data-status-shown="${statusShown}" data-status-hidden="${statusHidden}">
  <label class="tds-label" for="${id}">${label}</label>
  ${hint ? `<p class="tds-label__hint" id="${hintId}">${typeof hint === 'string' ? hint : 'Hint text'}</p>` : ''}
  ${error ? `<p id="${errorId}" class="tds-error-message">${typeof error === 'string' ? error : 'Enter a password'}</p>` : ''}
  <div class="tds-input__wrapper tds-password-input__wrapper${error ? ' tds-input--error' : ''}">
    <input ${inputAttributes} />
    <div class="tds-password-input__sr-status tds-visually-hidden" aria-live="polite"></div>
    <div class="tds-password-toggle-icon-wrapper">
      <button class="tds-button tds-button--icon-neutral tds-password-toggle-icon" type="button" data-module="tds-button" aria-controls="${id}" aria-label="${toggleLabel}" aria-pressed="${isOpened ? 'true' : 'false'}">
        <span class="tds-icon">
          <span data-password-icon="show"${isOpened ? ' hidden' : ''}>
          ${eyeIcon}
          </span>
          <span data-password-icon="hide"${isOpened ? '' : ' hidden'}>
          ${eyeCrossedIcon}
          </span>
        </span>
      </button>
